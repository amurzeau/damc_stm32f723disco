...
void CodecAudio::start() {
	...
	BSP_AUDIO_IN_OUT_Init(INPUT_DEVICE_INPUT_LINE_1, OUTPUT_DEVICE_HEADPHONE1, 48000, 16, 2, 91, 80);
...
void CodecAudio::processAudioInterleavedOutput(const int16_t* data_input, size_t nframes) {
	...
	out_buffer.writeOutBuffer(dma_read_offset, (uint32_t*) data_input, nframes);
}
